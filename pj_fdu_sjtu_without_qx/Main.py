...
def eval_epoch(model, validation_data, pred_loss_func, opt):
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            #     print(f'jump!')
            # else:
            #     print(f'input type is zero at pos? {input_type.gather(1, pos).squeeze(-1)}')

            for i in range(max_len-max_pos-1):
                """ forward """
                enc_out, prediction = model(input_type, input_time)
                ...
                pos += 1
                # if max(pos.squeeze(-1)) == max_len:
                #     break
                future_mask = future_mask.clone().scatter_add_(1, pos, torch.ones_like(pos).type_as(future_mask))
                # print(f'next future mask: {future_mask.sum()}')

                # input_type = input_type * future_mask
                # input_time = input_time * future_mask
...
def main():
    ...
    parser.add_argument('-scaletimeloss', type=float, default=1)
