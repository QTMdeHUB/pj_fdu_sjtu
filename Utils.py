...
def log_likelihood(model, data, time, types):
    ...
    non_pad_mask = get_non_pad_mask(types).squeeze(2)  # non_pad_mask.shape: bs*len

    # types.shape: bs * len_seq
    # types_mask.shape: bs * len_seq * num_types
...
def type_loss_train(prediction, types, loss_func):
...
def type_loss_test(prediction, types, loss_func):
    ...
    loss = torch.sum(loss)
    return loss, correct_num, pred_type + 1
